	...
	<div class="col-xs-12">		 
		<div class="card">
			<div class="card-header">
				<h4 class="card-title">Списък товарителници</h4>
			</div>             
							...
							<thead>
								...
								<tr>
									<form id="filter-form" method="get" action="{{ route('sales_data_file_list', ['id', 'DESC']) }}">
										...
										<td><a href="{{ route('sales_data_file_list_init', ['order'=>'id', 'direction' => 'DESC']) }}" class="btn btn-warning">Изчисти</a></td>
									</form>
								</tr>
							</thead>
						</table>
  ...
  $('input[name="date"]').on('cancel.daterangepicker', function(ev, picker) {
	...
	$('.sortable').on('click', function(){
		...
		if ($(this).find('i').attr('class') == 'fas fa-angle-down') {
			...
			direction = 'ASC';
			window.location.href = host+'/sales-data-file-list-init/'+ order + '/' + direction + '?date=' + filters.date + '&filename=' + filters.filename;
		}
