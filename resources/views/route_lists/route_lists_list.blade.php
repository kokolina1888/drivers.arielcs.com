...
@section('title', 'Пътни листове - списък')
			...
			<div class="card-body collapse in">
				<div class="card-block card-dashboard">
					<div class="table-responsive">
						<table class="table">							
							<form id="filter-form" method="get" action="{{ route('route_lists_list', ['id', 'DESC']) }}">
								<thead>
									<tr>
										<th>Филтрирай:</th>
										<th>дата от / до</th>
									...
									<tr>
										...
										<td>
											...
											<a href="{{ route('route_lists_list', ['order'=>'id', 'direction' => 'DESC']) }}" class="btn btn-warning">Изчисти</a>
										</td>
									</tr>
								</thead>
							</form>
								...
								<tbody>									
									@if( $route_lists->count() > 0 )
										...
										@foreach( $route_lists as $rl )
											<tr>
												<td>{{ $num++ }}</td>
												@if( $rl->is_handentered == 0 )
												@if( isset($rl->second_driver) )
												<td>
													@if($rl->first_driver->status )
														@if( $rl->second_driver )
															@if( $rl->truck->status )
													<form action="{{ route('route_list_edit', $rl->id) }}" method="post">
														{{ csrf_field() }}
														<input type="hidden" name="edit" value="{{ $filters['date']?? $init_date_range }}">
												...
												@endif
												<td>
													<form action="{{ route('route_list_destroy', $rl->id) }}" method="post">
														{{ csrf_field() }}
														<input type="hidden" name="delete_date" value="{{ $filters['date']?? $init_date_range }}">
														<input type="hidden" name="truck" value="{{ $filters['truck']??'' }}">
														<input type="hidden" name="first_driver" value="{{ $filters['first_driver']??'' }}">
														<input type="hidden" name="second_driver" value="{{ $filters['second_driver']??'' }}">
														<input type="hidden" name="order_number" value="{{ $filters['order_number']??'' }}">
